...
const ChatBotSidebar = () => {
	const { chats, currentChatId, searchQuery, setSearchQuery, createNewChat, selectChat } = useChatStore();
	...
	return (
		<motion.div initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.3 }} className={styles.sidebar}>
			{/* Logo Section */}
			<div className={styles.logoSection}>
				<Link to="/dashboard" className={styles.logoContainer}>
					<img src="/logo.png" alt="BrainBits Logo" className={styles.logoIcon} />
				</Link>
			</div>
					...
					{filteredChats.length === 0 ? (
						...
						filteredChats.map((chat) => (
							<motion.button
								...
								className={`${styles.chatItem} ${currentChatId === chat.id ? styles.chatItemActive : styles.chatItemInactive}`}
								whileHover={{ x: 2 }}
